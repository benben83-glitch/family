...
export function DeleteAlbumButton({ albumId }: { albumId: string }) {
  ...
  if (confirming) {
    return (
      <div className="flex items-center gap-2 text-sm">
        ...
        <button type="button" onClick={() => setConfirming(false)} className="rounded-full bg-card border border-border text-xs px-3 py-1">
          Annuler
        </button>
  ...
  return (
    <button type="button" onClick={() => setConfirming(true)} className="text-sm text-muted hover:text-red-700 transition-colors">
      Supprimer l&apos;album
    </button>
  );
}
